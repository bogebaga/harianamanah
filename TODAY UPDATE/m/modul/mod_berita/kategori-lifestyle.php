...
<ul class="navbar sub-rubrik">
  <?php
  $kalam = mysql_query("SELECT * FROM menu WHERE id_parent = (SELECT id_menu FROM menu WHERE nama_menu = 'Lifestyle') ORDER BY nama_menu");
...
<section class="container-fluid" style="background-color:white;">
		<section class="headline row">
			<?php
				$terkini=mysql_query("SELECT * FROM berita b JOIN (kategori k JOIN menu m ON k.id_kategori = m.id_menu) ON b.id_kategori = k.id_kategori WHERE headline='Y' AND m.id_parent = '13' ORDER BY id_berita DESC LIMIT 1");
		...
		<section class="daftar-artikel">
			<?php
			$_digit = 5;
			$artikel=mysql_query("SELECT * FROM berita b JOIN (kategori k JOIN menu m ON k.id_kategori = m.id_menu) ON b.id_kategori = k.id_kategori WHERE m.id_parent = '13' ORDER BY id_berita DESC LIMIT $_digit");
		...
		<section class="daftar-artikel">
			<?php
			$artikel=mysql_query("SELECT * FROM berita b JOIN (kategori k JOIN menu m ON k.id_kategori = m.id_menu) ON b.id_kategori = k.id_kategori WHERE m.id_parent = '13' ORDER BY id_berita DESC LIMIT 6, 5");
		...
		$(document).ready(function(){
			...
			$(window).scroll(function(){
				...
				if($(window).scrollTop()+nearbottom >= $(document).height() - $(window).height() && loadMore)
				{
					loadMore = false;
					$.ajax({
						url: 'more.php?kategori=lifestyle&urut='+$('.artikle-text:last').attr('kode'),
						beforeSend: function()
						{
							$('#more').show();
						},
