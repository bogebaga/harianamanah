...
$output .= $margin . '		<tr><th>Mata Uang</th><th align="left">Jual</th><th align="left">Beli</th></tr>' . "\n";
$rowclass = 'row1';
foreach ($nkurs['data'] as $key => $val) {
	if (in_array ($key, $nkurs['curr'])) {
		if ($rowclass == 'row1'){ $rowclass = 'row2'; }else{ $rowclass = 'row1'; }
		$output .= $margin . '		<tr><td align="center" class="curr ' .$rowclass . '">' . $key . '</td><td align="left" class="' . $rowclass . '">' . number_format($val[0], 2) . '</td><td align="left" class="' . $rowclass . '">' . number_format($val[1], 2) . '</td></tr>' . "\n";
	}
}
$output .= $margin . '	</table>' . "\n";
$output .= $margin . '	<cite>update: ' . $nkurs["remotelastupdate"] . '</cite>' . "\n";
$output .= $margin . '</div>' . "\n";
echo $output;
