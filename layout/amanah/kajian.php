<?php
  $terkini=mysql_query("SELECT * FROM menu WHERE id_menu='18'");
  $nama_menu = mysql_fetch_array($terkini);
?>
<!-- /////////////////////////////////////////Content -->
<div id="page-content" class="index-page container" style="background-color:#ececec;">
    <h1 style="color:#f44336;border-bottom:1px solid #f44336"><?php echo $nama_menu['nama_menu']?></h1>
    <div id="sidebar" style="padding:0;background:#ECECEC;">
      <div class="col-xs-12" style="padding:0;float:none;margin-bottom:25px">
        ...
        while($row = mysql_fetch_array($rubrik)){
          ...
          <span class='kategori-grid' style='background:rgba(244, 67, 54, 0.8)'><a style='color:#fff;' href='kategori-$row[id_kategori]-$row[kategori_seo]'>$row[nama_kategori]</a></span>
      ...
      <div class="col-xs-3" style="padding-left:0;">
        <div style="padding:10px;background-color: #f44336; background-image: -webkit-linear-gradient(147deg, #f44336 0%, #c6453c 74%); background-image: -moz-linear-gradient(147deg, #f44336 0%, #c6453c 74%); background-image: -o-linear-gradient(147deg, #f44336 0%, #c6453c 74%); background-image: linear-gradient(147deg, #f44336 0%, #c6453c 74%);">
          <ul>
          <h1 style="color:#fff;border-bottom:1px solid #fff;margin:0 0 15px 0;"><?php echo $nama_menu['nama_menu']?></h1>
          <?php
            $submenu = mysql_query("SELECT * FROM menu WHERE id_parent='18'");
      ...
      <div class="col-xs-9 news-category" style="background:#fff;">
          <!-- <h1 style="color:#f44336;border-bottom:1px solid #f44336"><?php echo $nama_menu['nama_menu']?></h1> -->
          <?php
          $terkini=mysql_query("SELECT * FROM berita b JOIN (kategori k JOIN menu m ON k.id_kategori = m.id_menu ) ON b.id_kategori = k.id_kategori WHERE  m.id_parent = '18' AND b.id_berita < $id_berita ORDER BY id_berita DESC LIMIT 33");
            ...
            <article>
              <div class='badges-cate'>
                <span style='background:#f44336'><a style='color:#fff;' href='kategori-$t[id_kategori]-$t[kategori_seo]'>$t[nama_kategori]</a></span>
              </div>
              <a href='berita-$t[judul_seo]'>
                <img src='http://harianamanah.com/foto_berita/$t[gambar]' alt=''>
              </a>
              <span style='font-size:10px;'>".$tgl." | ".$jam."</span>
              <a href='berita-$t[judul_seo]' class='captions' style='min-height:72px;'>$hasil</a>
            </article>"; }
          ?>
        </div>
