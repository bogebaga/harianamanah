<?php
  $terkini=mysql_query("SELECT * FROM menu WHERE id_menu='8'");
    ...
    <div id="sidebar" style="padding:0;background:#ECECEC;">
      <div class="col-xs-12" style="padding:0;float:none;margin-bottom:25px">
        ...
        while($row = mysql_fetch_array($rubrik)){
          ...
          <span class='kategori-grid' style='background:rgba(6, 55, 94, 0.8)'><a style='color:#fff;' href='kategori-$row[id_kategori]-$row[kategori_seo].html'>$row[nama_kategori]</a></span>
      ...
      <div class="col-xs-3" style="padding-left:0;">
        <div style="padding:10px;background-color: #06375e;background-image: -webkit-linear-gradient(107deg, #06375e 25%, #2b4a5f 100%);background-image: -moz-linear-gradient(107deg, #06375e 25%, #2b4a5f 100%);background-image: -o-linear-gradient(107deg, #06375e 25%, #2b4a5f 100%);background-image: linear-gradient(107deg, #06375e 25%, #2b4a5f 100%);" >
          <ul>
          <h1 style="color:#fff;border-bottom:1px solid #fff;margin:0 0 15px 0;"><?php echo $nama_menu['nama_menu']?></h1>
          <?php
            $submenu = mysql_query("SELECT * FROM menu WHERE id_parent='8'");
      ...
      <div class="col-xs-9 news-category" style="background:#fff;">
          ...
          <br>
          <?php
          $terkini=mysql_query("SELECT * FROM berita b JOIN (kategori k JOIN menu m ON k.id_kategori = m.id_menu ) ON b.id_kategori = k.id_kategori WHERE  m.id_parent = '8' AND b.id_berita < $id_berita ORDER BY id_berita DESC LIMIT 33");
            ...
            <article>
              ...
              <div class='badges-cate'>
                <span><a style='color:#fff;' href='kategori-$t[id_kategori]-$t[kategori_seo].html'>$t[nama_kategori]</a></span>
                <span>".$tgl." | ".$jam."</span>
              </div>
