<?php
  $terkini=mysql_query("SELECT * FROM menu WHERE id_menu='14'");
  $nama_menu = mysql_fetch_array($terkini);
?>
<!-- /////////////////////////////////////////Content -->
<div id="page-content" class="index-page container" style="background-color:#ececec;">
<h1 style="color:#2196F3;border-bottom:1px solid #2196F3"><?php echo $nama_menu['nama_menu']?></h1>
    <div id="sidebar" style="padding:0;background:#ECECEC;">
      <div class="col-xs-12" style="padding:0;float:none;margin-bottom:25px">
        <div class='flex-container'>
        <?php
        $rubrik=mysql_query("SELECT * FROM berita b JOIN (kategori k JOIN menu m ON k.id_kategori = m.id_menu ) ON b.id_kategori = k.id_kategori WHERE  m.id_parent = '14' ORDER BY id_berita DESC LIMIT 5");
        $i=0;
        while($row = mysql_fetch_array($rubrik)){
          ...
          <span class='kategori-grid' style='background:rgba(33, 150, 243, 0.8)'><a style='color:#fff;' href='kategori-$row[id_kategori]-$row[kategori_seo].html'>$row[nama_kategori]</a></span>
      ...
      <div style="padding:10px;background-color: #2196f3;background-image: -webkit-linear-gradient(89deg, #2196f3 0%, #3378af 100%);background-image: -moz-linear-gradient(89deg, #2196f3 0%, #3378af 100%);background-image: -o-linear-gradient(89deg, #2196f3 0%, #3378af 100%);background-image: linear-gradient(89deg, #2196f3 0%, #3378af 100%);">
        <ul>
        <h1 style="color:#fff;border-bottom:1px solid #fff;margin:0 0 15px 0;"><?php echo $nama_menu['nama_menu']?></h1>
        <?php
          $submenu = mysql_query("SELECT * FROM menu WHERE id_parent='14'");
      ...
      <div class="col-xs-9 news-category" style="background:#fff;">
          ...
          <br>
          <?php
          $terkini=mysql_query("SELECT * FROM berita b JOIN (kategori k JOIN menu m ON k.id_kategori = m.id_menu ) ON b.id_kategori = k.id_kategori WHERE  m.id_parent = '14' AND b.id_berita < $id_berita ORDER BY id_berita DESC LIMIT 33");
            ...
            <article>
              ...
              <div class='badges-cate'>
                <span style='background:#2196F3;'><a style='color:#fff;' href='kategori-$t[id_kategori]-$t[kategori_seo].html'>$t[nama_kategori]</a></span>
                <span>".$tgl." | ".$jam."</span>
              </div>
		...
		<!-- <script>
			...
			$(window).scroll(function(){
				...
				if(($(window).scrollTop()+nearbottom >= $(document).height()-$(window).height()) && loadMore)
				{
					loadMore = false;
					$.ajax({
						url: 'more-web.php?kategori=komunitas&urut_1='+$('li[data-kategori=muslimah]:last').attr('kode')+'&urut_2='+$('li[data-kategori=gen]:last').attr('kode')+'&urut_3='+$('li[data-kategori=ormas]:last').attr('kode'),
						beforeSend: function()
						{
							$('#more').show();
						},
